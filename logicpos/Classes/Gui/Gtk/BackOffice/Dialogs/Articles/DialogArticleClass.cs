...
{
    class DialogArticleClass : BOBaseDialog
    {
        ...
        {
            this.Title = Utils.GetWindowTitle(resources.CustomResources.GetCustomResources(GlobalFramework.Settings["customCultureResourceDefinition"], "window_title_edit_dialog_article_class"));
            SetSizeRequest(400, 440);
            InitUI();
            InitNotes();
            ShowAll();
        }
        ...
        private void InitUI()
        {
            try
            {
                ...
                _crudWidgetList.Add(new GenericCRUDWidgetXPO(boxAcronym, _dataSourceRow, "Acronym", SettingsApp.RegexAlfaAcronym1Char, true));

                //WorkInStock
                CheckButton checkButtonWorkInStock = new CheckButton(resources.CustomResources.GetCustomResources(GlobalFramework.Settings["customCultureResourceDefinition"], "global_work_in_stock"));
